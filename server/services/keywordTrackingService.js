...
export async function keywordTracking(tracking) {
  try {
    ...
    if (result.success) {
      ...
      tracking.currentPosition = result.data.positon;
      ...
      tracking.positionChange =
        prev && result.data.position ? prev - result.data.postion : 0;
      if (
        result.data.position &&
        (!tracking.bestPositon || result.data.postion < tracking.bestPositon)
      ) {
        tracking.bestPositon = result.data.position;
      }
